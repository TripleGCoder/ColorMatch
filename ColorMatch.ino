...
byte userIndex = 0;   // which element of currentPattern the user is answering
    ...
    if (pressedButton == BUTTONS[1][0])
      {
        ...
        Serial.println("Completed State Start");      //Debugging Flag
...
//In LEVEL 3, Start with 4 light pattern then from 4 -> 9 
  ...
  case UPDATE_PTRN: {
    ...
    Serial.println("Completed State: UPDATE_PTRN");      //Debugging Flag
...
//------------------------DISPLAY PTRN----------------------------------|
  case DISPLAY_PTRN: {
    ...
    Serial.println("Completed State: DISPLAY_PTRN");      //Debugging Flag
    break;
  }
...
//------------------------AWAIT RES----------------------------------|
  case GET_RES: {
    ...
    if (pressedButton != NO_KEY)
    {
      ...
      Serial.println("Completed State: GET_RES");      //Debugging Flag
      break;
    }
    else if (timeDone)
    {
    ...
    currentState = GET_RES;
    Serial.println("NO Response Yet");      //Debugging Flag
...
//------------------------CHECK RES----------------------------------|
  case CHECK_RES: {
    if (!timeDone && pressedButton != NO_KEY && pressedButton == currentPattern[userIndex])
      {
        corAnsw = true;
      }
      else
    ...
    Serial.println("Completed State: CHECK_RES");      //Debugging Flag
    break;
  }
...
//------------------------UPDATE_GAME----------------------------------|
  case UPDATE_GAME: {
    if(corAnsw)
    {
      ...
      if (lastRound)
      {
        userIndex = 0;       // reset for next round input
        if (lastLevel)
        {
    ...
    Serial.println("Completed State: UPDATE_GAME");      //Debugging Flag
    break;
  }
...
//------------------------GAME WON----------------------------------|
  case GAME_WON: {
    for (int i=0; ;i++)
    {
      allSync = true;
      flash_leds();
      Serial.println("GAMEWON");      //Debugging Flag
    }
    break;
  }
...
//------------------------GAME LOST----------------------------------|
  case GAME_LOST: {
      enRedLED = true;
      flash_leds();
      Serial.println("GAMELOST");      //Debugging Flag
